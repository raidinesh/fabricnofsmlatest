...
func (handler *Handler) triggerNextState(msg *pb.ChaincodeMessage, send bool, errc chan error) {
	if send == true {

		chaincodeLogger.Debugf("[%s]send state message %s", shorttxid(msg.Txid), msg.Type.String())
...
func (handler *Handler) handleInit(msg *pb.ChaincodeMessage, errc chan error) {
	...
	go func() {
		...
		defer func() {
			handler.triggerNextState(nextStateMsg, send, errc)
		}()
...
func (handler *Handler) handleInvokeChaincode(chaincodeName string, args [][]byte, txid string) pb.Response {
	...
	// Incorrect chaincode message received
	chaincodeLogger.Debugf("[%s]Incorrect chaincode message %s received. Expecting %s or %s", shorttxid(responseMsg.Txid), responseMsg.Type.String(), pb.ChaincodeMessage_RESPONSE, pb.ChaincodeMessage_ERROR)
	return pb.Response{
		Status:  ERROR,
...
func (handler *Handler) handleMessage(msg *pb.ChaincodeMessage, errc chan error) error {
	...
	chaincodeLogger.Debugf("[%s]Handling ChaincodeMessage of type: %s(state:%s)", shorttxid(msg.Txid), msg.Type.String(), handler.FSMState)

	if handler.FSMState == "ready" {
		if !(msg.Type == pb.ChaincodeMessage_REGISTERED) || !(msg.Type == pb.ChaincodeMessage_READY) {

			chaincodeLogger.Debugf("Received %s, communicated (state:%s)", msg.Type, handler.FSMState)

			if msg.Type == pb.ChaincodeMessage_RESPONSE {
				if err := handler.sendChannel(msg); err != nil {
					chaincodeLogger.Errorf("[%s]error sending %s (state:%s): %s", shorttxid(msg.Txid), msg.Type.String(), handler.FSMState, err)
					return err
				} else {
					chaincodeLogger.Debugf("[%s]Received %s, communicated (state:%s)", shorttxid(msg.Txid), msg.Type.String(), handler.FSMState)
					return nil
				}

			} else if msg.Type == pb.ChaincodeMessage_ERROR {
				if err := handler.sendChannel(msg); err != nil {
					chaincodeLogger.Errorf("[%s]error sending %s (state:%s): %s", shorttxid(msg.Txid), msg.Type.String(), handler.FSMState, err)
					return err
				} else {
					chaincodeLogger.Debugf("[%s]Error Received %s, communicated (state:%s)", shorttxid(msg.Txid), msg.Type.String(), handler.FSMState, err)
					return nil
				}

			} else if msg.Type == pb.ChaincodeMessage_INIT {
				...
				return nil
			} else if msg.Type == pb.ChaincodeMessage_TRANSACTION {
				chaincodeLogger.Debugf("[%s]Received %s, invoking transaction on chaincode(state:%s)", shorttxid(msg.Txid), msg.Type.String(), handler.FSMState)
	...
	if handler.FSMState == "established" {
		if msg.Type == pb.ChaincodeMessage_READY {
			handler.FSMState = "ready"
			return nil
		} else {
			errStr := fmt.Sprintf("[%s]Chaincode handler FSM cannot handle message (%s) with payload size (%d) while in state: %s", msg.Txid, msg.Type.String(), len(msg.Payload), handler.FSMState)
	...
	if handler.FSMState == "created" {
		if msg.Type == pb.ChaincodeMessage_REGISTERED {
			handler.FSMState = "established"
			return nil
		} else {
			errStr := fmt.Sprintf("[%s]Chaincode handler FSM cannot handle message (%s) with payload size (%d) while in state: %s", msg.Txid, msg.Type.String(), len(msg.Payload), handler.FSMState)
			err := errors.New(errStr)
