...
type Handler struct {
	...
	//used to store all allowed messages in ready state
...
func (handler *Handler) initeventMap() {
	handler.eventMap = make(map[string]bool)
	handler.eventMap[pb.ChaincodeMessage_PUT_STATE.String()] = true
	handler.eventMap[pb.ChaincodeMessage_DEL_STATE.String()] = true
	handler.eventMap[pb.ChaincodeMessage_INVOKE_CHAINCODE.String()] = true
	handler.eventMap[pb.ChaincodeMessage_COMPLETED.String()] = true
	handler.eventMap[pb.ChaincodeMessage_GET_STATE.String()] = true
	handler.eventMap[pb.ChaincodeMessage_GET_STATE_BY_RANGE.String()] = true
	handler.eventMap[pb.ChaincodeMessage_GET_QUERY_RESULT.String()] = true
	handler.eventMap[pb.ChaincodeMessage_GET_HISTORY_FOR_KEY.String()] = true
	handler.eventMap[pb.ChaincodeMessage_QUERY_STATE_NEXT.String()] = true
	handler.eventMap[pb.ChaincodeMessage_QUERY_STATE_CLOSE.String()] = true
	handler.eventMap[pb.ChaincodeMessage_ERROR.String()] = true
	handler.eventMap[pb.ChaincodeMessage_RESPONSE.String()] = true
	handler.eventMap[pb.ChaincodeMessage_INIT.String()] = true
	handler.eventMap[pb.ChaincodeMessage_TRANSACTION.String()] = true
}
...
func (handler *Handler) triggerNextState(msg *pb.ChaincodeMessage, send bool) {
	if send {
		handler.serialSendAsync(msg, handler.errc)
	}
}
...
func (handler *Handler) triggerNextStateSync(msg *pb.ChaincodeMessage) {
	...
	if err = handler.serialSend(msg); err != nil {
		chaincodeLogger.Errorf("[%s]Error sending   message, ending stream: %s", shorttxid(msg.Txid), err)
		handler.errc <- err
	}
}
...
func (handler *Handler) processStream() error {
	...
	handler.errc = make(chan error, 1)
	for {
		in = nil
		err = nil
		//nsInfo = nil
	...
	if handler.readyNotify != nil {
		...
		if handler.chaincodeSupport.userRunsCC {
			if val {
				...
				chaincodeLogger.Debugf("[%s]Entered state %s", shorttxid(ccMsg.Txid), handler.FSMState)
				handler.FSMState = readystate
				chaincodeLogger.Debugf("[%s]Updated to state %s", shorttxid(ccMsg.Txid), handler.FSMState)
...
func (handler *Handler) beforeRegisterEvent(msg *pb.ChaincodeMessage, state string) {
	...
	handler.FSMState = establishedstate
	handler.notifyDuringStartup(true)
	chaincodeLogger.Debugf("Changed state = %s for Event = %s", handler.FSMState, pb.ChaincodeMessage_REGISTER.String())
}
...
func (handler *Handler) HandleMessage(msg *pb.ChaincodeMessage) error {
	chaincodeLogger.Debugf("[%s]Fabric side Handling ChaincodeMessage of type: %s in state %s", shorttxid(msg.Txid), msg.Type, handler.FSMState)

	if handler.FSMState == readystate {

		if _, ok := handler.eventMap[msg.Type.String()]; !ok {

			return fmt.Errorf("[%s]Chaincode handler validator FSM cannot handle message (%s) with payload size (%d) while in state: %s", msg.Txid, msg.Type.String(), len(msg.Payload), handler.FSMState)
	...
	if handler.FSMState == createdstate {
		if msg.Type != pb.ChaincodeMessage_REGISTER {
			return fmt.Errorf("[%s]Chaincode handler validator FSM cannot handle message (%s) with payload size (%d) while in state: %s", msg.Txid, msg.Type.String(), len(msg.Payload), handler.FSMState)
		} else {
			handler.beforeRegisterEvent(msg, createdstate)

		}
		return nil
	}

	if handler.FSMState == establishedstate {

		if msg.Type != pb.ChaincodeMessage_READY {
			return fmt.Errorf("[%s]Chaincode handler validator FSM cannot handle message (%s) with payload size (%d) while in state: %s", msg.Txid, msg.Type.String(), len(msg.Payload), handler.FSMState)
		} else {
			chaincodeLogger.Debugf("[%s]Entered state %s", shorttxid(msg.Txid), handler.FSMState)
			handler.FSMState = readystate
			//handler.FSM.current = readystate
			chaincodeLogger.Debugf("[%s]Updated to state %s", shorttxid(msg.Txid), handler.FSMState)
			handler.notify(msg)
